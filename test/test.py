...
def _make_testcase(testcase):
    ...
    def test_method(self):
        ...
        expected_output: List[List[int]] = testcase['output']
        ...
        if matrix == expected_output:
            print(f'✅ Result    : Passed with answer {matrix}')
        else:
            print(f'❌ Result    : Expected {expected_output}, got {matrix}')
...
class TestSolution(unittest.TestCase):
    ...
    def _run_case(self, matrix: List[List[int]]) -> List[int]:
        ...
        return self.__solution.rotate(matrix=matrix)


class RotateImageTextTestResult(unittest.TextTestResult):
...
if __name__ == '__main__':
    unittest.main(
        verbosity=2,
        testRunner=unittest.TextTestRunner(resultclass=RotateImageTextTestResult)
    )
